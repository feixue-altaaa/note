...
| 存储引擎 | 特点                                                         |
| :------: | :----------------------------------------------------------- |
|  InnoDb  | 支持事务； 行级锁，提高并发访问性能； 支持外键FOREIGN KEY约束，保证数据的完整性和一致性； |
...
> **相同点：**
>
> 1. 都是索引：聚集索引和二级索引都是数据库中用于加速数据检索的数据结构。
> 2. 提高查询速度：两种索引都可以显著提高数据查询速度，避免全表扫描。
> 3. 逻辑顺序：它们都可以根据索引键值的逻辑顺序对数据进行排序。
>
> **不同点：**
>
> 1. 存储数据方式：聚集索引将数据行存储在索引中，按照索引键值的顺序进行排序。数据行只存在于聚集索引中，不存在于其他地方。而二级索引存储的是指向实际数据行的指针（如聚集索引键值或行ID），而不是数据行本身。
> 2. 索引数量：一个表只能有一个聚集索引，因为数据行本身只能按照一种顺序进行存储。但是，一个表可以有多个二级索引。
> 3. 效率：聚集索引的查询效率通常比二级索引高，因为在聚集索引中，索引和数据行是在一起的，无需额外的数据查找。而在二级索引中，查询需要执行两个步骤：首先通过索引找到对应的聚集索引键值或行ID，然后再通过这个键值或行ID找到实际的数据行。
> 4. 范围查询：对于范围查询，聚集索引的效果更好，因为它们按顺序存储数据行，可以更高效地执行范围扫描。而二级索引可能需要多次查找实际数据行，对于范围查询效率较低。
> 5. 更新开销：聚集索引的更新开销通常较大，因为它需要维护数据行的物理顺序。而在二级索引中，仅需要更新索引及指针，开销较小。
>
